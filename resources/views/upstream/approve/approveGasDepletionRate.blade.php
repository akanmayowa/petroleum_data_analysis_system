<div class="table-responsive">
    <h5 style="margin-left: 5px; color: #aaa"> Pending Gas Reserves Addition Depletion Rate  
        <a data-toggle="tooltip" onclick="showmodal('appGasADRmodal')" class="btn btn-primary waves-effect waves-light approve-btn pull-right" title="Approve All Pending Data" style="color: #fff">  <i class="fa fa-check"> </i> Approve Data </a>
    </h5>
            ...
            <tbody>
                @if($rate)
                    @foreach($rate as $rated)
                        <tr>            
                            <th>
                                <input type="checkbox" onclick="setValue({{$rated->id}}, '\App\\up_gas_reserve_addition_depletion_rate')" class="check_tabs" id="tab_{{$rated->id}}" name="tab_{{$rated->id}}" value="0">
                            </th>  
                            ...
                            <th class="tb-row-height">{{number_format($rated->prev_gas_reserve, 2)}}</th>
                            <th class="tb-row-height">{{number_format($rated->curr_gas_reserve, 2)}}</th>
    ...
    $(function()
    {
        ...
        {    
            ...
            { 
                formData = 
                {
                    model_name:'\App\\up_gas_reserve_addition_depletion_rate',
                    name:'Gas Addition Depletion Rate',
                    stage_id:1,
                    _token:'{{csrf_token()}}'
                }
