...
<form id="" action="{{url('/upstream/add_oil_facility')}}" method="POST">
      {{ csrf_field() }}
        <input type="hidden" class="form-control" id="id" name="id" value="{{$GAS_FACILITY->id}}" required>
        ...
        <?php 
            ...
            $one_lic = \App\up_license_status::where('id', $GAS_FACILITY->license_status)->first();            
        ?>
      ...
      <div class="form-group row">
        <label for="location_id_face" class="col-sm-2 col-form-label"> Location </label>
        <div class="col-sm-10">
            <select class="form-control" name="location_id" id="location_id_face" disabled>
      ...
      <div class="form-group row">
        ...
        <div class="col-sm-4">
            <select class="form-control" name="license_status" id="license_status" disabled="">
                @if($one_lic) <option value="{{$one_lic->id}}"> {{$one_lic->license_status_name}} </option> @else <option value=""> N/A </option>  @endif
            </select>
        </div>
      </div>
