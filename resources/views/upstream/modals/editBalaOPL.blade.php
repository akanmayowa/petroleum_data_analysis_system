<form id="upOPLForm" action="{{url('/upstream')}}" method="POST">
          {{ csrf_field() }}
          ...
          <div class="form-group row">
            <label for="year_ople" class="col-sm-2 col-form-label"> Year </label>
            <div class="col-sm-4">
                <input type="text" class="form-control" placeholder="Year YYYY" name="year" id="year_ople" value="{{$OPL_->year}}" required readonly>
            </div>
  ...
  <script>
    $(function()
    {        
        $("#upOPLForm").on('submit', function(e)
        {   
            e.preventDefault();            
            editForm('upOPLForm', "{{url('/upstream')}}", 'editbala_opl');

            displayOPL();
        });

        $('#year_ople').datepicker(
        {
