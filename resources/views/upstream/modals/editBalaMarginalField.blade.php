<form id="upMFieldForm" action="{{url('/upstream')}}" method="POST">
          @php 
                $one_com = \App\company::where('id', $M_FIELD_->company_id)->first();       
                $all_com = \App\company::orderBy('company', 'asc')->get();
                $one_fie = \App\field::where('id', $M_FIELD_->field_id)->first();       
                $all_fie = \App\field::orderBy('field', 'asc')->get();
          @endphp
          {{ csrf_field() }}
...
<script>    
    ...
    $(function()
    {    
        $("#upMFieldForm").on('submit', function(e)
        {   
            e.preventDefault();            
            editForm('upMFieldForm', "{{url('/upstream')}}", 'editbala_m_field');
