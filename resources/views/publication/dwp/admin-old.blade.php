                    ...
                    <div class="card-body">  
                        ...
                        <span class="text-muted m-b-30 font-14"> {{$section->title}} </span> 
                            ...
                            @if(\Auth::user()->role_obj->role_name == 'DWP Report Manager' || \Auth::user()->role_obj->role_name == 'Admin')
                                @if($section->status == 0) 
                                    <button type="button" class="btn btn-success pull-right" onclick="getId({{ $section->id }})" data-toggle="modal" data-target="#approve_{{$section->id}}" title="Approve This Sectionks" style="font-size: 10px; margin-left: 5px; color: #000"><i class="fa fa-check"> Approve</i></button> 
                                @endif

                                <button type="button" class="btn btn-default pull-right" id="" onclick="load_sections({{$section->id}})" data-toggle="tooltip" data-placement="left" title data-original-title="Edit This Section" style="font-size: 10px; background-color:#202020; color:#fff;"><i class="fa fa-pencil"> Edit</i></button>
                            @endif
                    </div>
                </div>
            </div> <!-- end col -->
...
@if($dwp)
    @foreach($dwp as $sections)
         ...
         <form class="form-horizontal" role="form" method="POST" action="{{route('approve_dwp')}}">
...
<script type="text/javascript">
    ...
    {  
        ...
        {
            var year = this.value;
            window.location="{{url('/publication/dwp')}}?year="+year;
        });
    });
    ...
    {   
        $('#editsection').load("{{url('publication')}}/modals/editDWPSection?id="+id);
