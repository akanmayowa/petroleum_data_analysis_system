<form id="pubForm" action="{{url('/publication/nogiar/update')}}" method="POST" enctype="multipart/form-data">
        ...
        <div id="section" class="col-md-12" style="border:#e1e1e1 thin solid; border-radius: 6px; border-bottom-color: #fff">
            ...
            <div style="background-color: #AEC6CF; color: #202020; margin: 0px -15px; padding: 10px 15px">  
                <table width="100%">
                    <tr>
                        <td width="40%"> Add NOGIAR Publications </td>
                        <td width="10%">
                  ...
                  <?php 
                  		$contents = \App\nogiar_publication_content::where('nogiar_id', $SECTION->id)->orderBy('id', 'asc')->get(); 
                  		$content_count = \App\nogiar_publication_content::where('nogiar_id', $SECTION->id)->count(); 	  $i = 0; 

                  		$lists = \App\nogiar_publication_list::where('nogiar_id', $SECTION->id)->orderBy('id', 'asc')->get(); 
                  		$list_count = \App\nogiar_publication_list::where('nogiar_id', $SECTION->id)->count(); 	  $j = 0; 
                  ?>
                  @if($contents)
