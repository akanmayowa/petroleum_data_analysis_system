 <form id="prodPermitVesselForm" action="{{url('/downstream')}}" method="POST">
          @php 
          ...
          {{ csrf_field() }}
           ...
           <div class="form-group row">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <input type="date" class="form-control" placeholder="" name="year" id="year" value="{{$PRO_VOL_PERM_VESS->year}}" required>
                </div>
            </div>
...
<script>
    $(function()
    {
        $("#prodPermitVesselForm").on('submit', function(e)
        {   
            e.preventDefault();            
            editForm('prodPermitVesselForm', "{{url('/downstream')}}", 'edit_imp_vessel');
